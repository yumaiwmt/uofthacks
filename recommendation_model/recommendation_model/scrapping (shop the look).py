...
class ShopTheLookDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            pos_img = Image.open(
                os.path.join(self.root, "images/products", pos_id + ".jpg")
            ).convert("RGB")

            neg_img = Image.open(
                os.path.join(self.root, "images/products", neg_id + ".jpg")
            ).convert("RGB")
            ...
            logger.debug(f"Error loading item {idx}: {e}")
            # Return a dummy batch instead of recursing
            # This shouldn't happen if validation was done
...
def main():
    try:
        ...
        loader = DataLoader(dataset, batch_size=8, shuffle=True, num_workers=0)  # num_workers=0 for Windows
